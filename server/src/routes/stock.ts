...
interface StockMovementRecord { _id: ObjectId; productId: ObjectId; productName: string; unit: string; type: StockMovementType; quantity: number; reason: string; createdByUserId: ObjectId; createdAt: Date }
...
stockRouter.get('/', async (_req: AuthenticatedRequest, res) => {
  const db = getDb()
  const [products, purchases, sales, adjustments, valuation] = await Promise.all([
  ...
  const stock = products.map((product) => { const id = product._id.toHexString(); const valued = valuationMap.get(id); return { id, name: String(product.name), unit: String(product.unit), isActive: Boolean(product.isActive), purchased: round(purchaseMap.get(id) ?? 0), sold: round(salesMap.get(id) ?? 0), adjusted: round(adjustmentMap.get(id) ?? 0), quantity: round((purchaseMap.get(id) ?? 0) - (salesMap.get(id) ?? 0) + (adjustmentMap.get(id) ?? 0)), stockValue: valued?.stockValue ?? 0, averageCost: valued?.averageCost ?? 0 } })
...
stockRouter.post('/adjustments', async (req: AuthenticatedRequest, res) => {
  ...
  const signedQuantity = round(direction === 'remove' ? -quantity : quantity); const movementId = new ObjectId(); const now = new Date(); const movement: StockMovementRecord = { _id: movementId, productId: product._id, productName: String(product.name), unit: String(product.unit), type, quantity: signedQuantity, reason, createdByUserId: req.user!._id, createdAt: now }
  await db.collection<StockMovementRecord>('stock_movements').insertOne(movement); await recordAuditEvent({ actorUserId: req.user!._id, actorRole: req.user!.role, action: 'create', entity: 'stock_movement', entityId: movementId.toHexString(), details: { productName: movement.productName, type, quantity: signedQuantity, reason } })
