...
stockRouter.post('/adjustments', async (req: AuthenticatedRequest, res) => {
  ...
  if (direction === 'remove') {
    ...
    const current = valuation.items.find((item) => item.productId === productId)?.quantity ?? 0
    if (quantity > current + 0.000001) return res.status(400).json({ message: `Only ${round(Math.max(0, current)).toFixed(2)} ${String(product.unit)} available. You cannot remove ${round(quantity).toFixed(2)} ${String(product.unit)}.` })
  }
