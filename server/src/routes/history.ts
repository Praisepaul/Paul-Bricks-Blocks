...
historyRouter.get('/', async (req: AuthenticatedRequest, res) => {
  ...
  const items: HistoryItem[] = [
    ...sales.map((item) => ({ id: String(item._id), type: 'sale' as const, number: String(item.invoiceNumber), title: `Sale to ${item.customerName}`, subtitle: `${item.productName} · ${item.quantity} ${item.unit}`, amount: Number(item.totalAmount), date: String(item.createdAt).slice(0, 10), createdAt: new Date(item.createdAt) })),
    ...purchases.map((item) => ({ id: String(item._id), type: 'purchase' as const, number: String(item.purchaseNumber), title: `Purchase from ${item.supplierName}`, subtitle: `${item.productName} · ${item.quantity} ${item.unit}`, amount: Number(item.totalAmount), date: String(item.createdAt).slice(0, 10), createdAt: new Date(item.createdAt) })),
