...
dashboardRouter.get('/today', async (_req: AuthenticatedRequest, res) => {
  ...
  const [sales, purchases, expenses, labour, unpaidBills] = await Promise.all([
    db.collection('sales').aggregate([{ $match: { createdAt: { $gte: new Date(`${today}T00:00:00.000Z`) } }, $group: { _id: null, total: { $sum: '$totalAmount' } } }]).toArray(),
    db.collection('purchases').aggregate([{ $match: { createdAt: { $gte: new Date(`${today}T00:00:00.000Z`) } }, $group: { _id: null, total: { $sum: '$totalAmount' } } }]).toArray(),
  ...
  return res.json({ today, sales: value(sales), purchases: value(purchases), expenses: value(expenses), labour: value(labour), unpaidBills: value(unpaidBills), unpaidBillCount: Number(unpaidBills[0]?.count ?? 0) })
})
