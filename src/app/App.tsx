...
function AuthenticatedApp({ roleLabel }: { roleLabel: string }) {
  ...
  return <div className="app-shell">
    ...
    <main className="page-content">{activeSection === 'dashboard' ? <Dashboard /> : <Placeholder section={activeSection} />}
    ...
    <nav className="bottom-nav" aria-label="Main navigation">{sections.map((section) => <Button key={section.id} variant={activeSection === section.id ? 'active' : 'nav'} onClick={() => setActiveSection(section.id)}>{section.label}</Button>)}</nav>
  </div>
}
...
function SignInScreen() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    catch (errorValue) { setError(errorValue instanceof Error ? errorValue.message : 'Unable to sign in.') }
    finally { setBusy(false) }
  }
  return <main className="auth-screen"><section className="auth-card"><p className="eyebrow">Paul Bricks & Blocks</p><h1>Sign in</h1><p>Use your business account to continue.</p>
    <form onSubmit={handleSubmit} className="auth-form"><label>Email<input type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Password<input type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>
      {error && <p className="error-text" role="alert">{error}</p>}<Button variant="primary" type="submit" disabled={busy}>{busy ? 'Signing in…' : 'Sign in'}</Button>
    </form></section></main>
}
...
function Dashboard() { return <><section className="welcome-card"><p className="eyebrow">Good morning</p><h2>What do you want to do?</h2><div className="action-grid"><Button variant="primary">+ New Sale</Button><Button variant="secondary">+ Add Purchase</Button><Button variant="secondary">+ Add Expense</Button><Button variant="secondary">+ Add Labour</Button></div></section><section className="summary-grid"><SummaryCard label="Today's Sales" value="₹0" /><SummaryCard label="Today's Expenses" value="₹0" /><SummaryCard label="Pending Payments" value="₹0" /></section></> }
...
function getSectionTitle(section: Section) { return sections.find((item) => item.id === section)?.label ?? 'Home' }
