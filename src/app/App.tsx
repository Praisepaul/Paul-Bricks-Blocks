  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div><p className="eyebrow">Paul Bricks & Blocks</p><h1>{getSectionTitle(activeSection)}</h1></div>
      ...
      <nav className="bottom-nav" aria-label="Main navigation">
        {sections.map((section) => <Button key={section.id} variant={activeSection === section.id ? 'active' : 'nav'} onClick={() => setActiveSection(section.id)}>{section.label}</Button>)}
      </nav>
    </div>
  )
}
...
function SignInScreen() {
  ...
  return (
    <main className="auth-screen"><section className="auth-card">
      <p className="eyebrow">Paul Bricks & Blocks</p><h1>Sign in</h1><p>Use your business account to continue.</p>
...
function ConfigurationNotice() { return <main className="auth-screen"><section className="auth-card"><p className="eyebrow">Setup needed</p><h1>Connect the business account</h1><p>Supabase is not configured yet. Add the local Vite Supabase environment values, then restart the development server.</p></section></main> }
...
function Dashboard() {
  return <><section className="welcome-card"><p className="eyebrow">Good morning</p><h2>What do you want to do?</h2><div className="action-grid"><Button variant="primary">+ New Sale</Button><Button variant="secondary">+ Add Purchase</Button><Button variant="secondary">+ Add Expense</Button><Button variant="secondary">+ Add Labour</Button></div></section><section className="summary-grid" aria-label="Business summary"><SummaryCard label="Today's Sales" value="₹0" /><SummaryCard label="Today's Expenses" value="₹0" /><SummaryCard label="Pending Payments" value="₹0" /></section></>
}
