...
const sections: Array<{ id: Section; label: string }> = [
  { id: 'dashboard', label: 'Home' }, { id: 'customers', label: 'Customers' }, { id: 'products', label: 'Products' }, { id: 'sales', label: 'Sales' }, { id: 'purchases', label: 'Purchases' }, { id: 'expenses', label: 'Expenses' },
...
function AuthenticatedApp({ roleLabel }: { roleLabel: string }) {
  ...
  return <div className="app-shell">
    ...
    <main className="page-content">{activeSection === 'dashboard' ? <Dashboard onNewSale={() => setActiveSection('sales')} onNewPurchase={() => setActiveSection('purchases')} onNewExpense={() => setActiveSection('expenses')} /> : activeSection === 'customers' ? <Customers /> : activeSection === 'products' ? <Products /> : activeSection === 'sales' ? <Sales /> : activeSection === 'purchases' ? <Purchases /> : activeSection === 'expenses' ? <Expenses /> : activeSection === 'users' && isOwner ? <Users /> : activeSection === 'settings' && isOwner ? <Settings /> : <Placeholder section={activeSection}/>}<div className="account-strip"><span>{user?.email ?? 'Signed in'}</span><Button variant="secondary" onClick={() => void signOut()}>Sign out</Button></div></main>
...
function Dashboard({ onNewSale, onNewPurchase, onNewExpense }: { onNewSale: () => void; onNewPurchase: () => void; onNewExpense: () => void }) { return <><section className="welcome-card"><p className="eyebrow">Good morning</p><h2>What do you want to do?</h2><div className="action-grid"><Button variant="primary" onClick={onNewSale}>+ New Sale</Button><Button variant="secondary" onClick={onNewPurchase}>+ Add Purchase</Button><Button variant="secondary" onClick={onNewExpense}>+ Add Expense</Button><Button variant="secondary">+ Add Labour</Button></div></section><section className="summary-grid"><SummaryCard label="Today's Sales" value="₹0"/><SummaryCard label="Today's Expenses" value="₹0"/><SummaryCard label="Pending Payments" value="₹0"/></section></> }
