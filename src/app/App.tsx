...
type Section = 'dashboard' | 'sales' | 'purchases' | 'labour' | 'bills' | 'history' | 'users'
...
function AuthenticatedApp({ roleLabel }: { roleLabel: string }) {
  ...
  const visibleSections = isOwner ? [...sections, { id: 'users' as Section, label: 'Users' }] : sections
  return <div className="app-shell">
    ...
    <main className="page-content">{activeSection === 'dashboard' ? <Dashboard /> : activeSection === 'users' && isOwner ? <Users /> : <Placeholder section={activeSection} />}
...
function getSectionTitle(section: Section) { return sections.find((item) => item.id === section)?.label ?? (section === 'users' ? 'Users' : 'Home') }
