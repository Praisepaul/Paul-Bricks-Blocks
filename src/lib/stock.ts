...
export async function createStockAdjustment(input: StockAdjustmentInput) { const response = await fetch(`${API_URL}/stock/adjustments`, { method: 'POST', headers: headers(), body: JSON.stringify(input) }); const body = (await response.json().catch(() => ({}))) as { movement?: unknown; message?: string }; if (!response.ok) throw new Error(body.message ?? 'Unable to update stock'); return body.movement }
