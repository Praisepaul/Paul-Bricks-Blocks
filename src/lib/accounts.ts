...
export function getSupplierAccount(idOrName: string): Promise<AccountStatement> { const query = encodeURIComponent(idOrName); const parameter = /^[a-f0-9]{24}$/i.test(idOrName) ? `id=${query}` : `name=${query}`; return request<AccountStatement>(`/accounts/supplier?${parameter}`) }
